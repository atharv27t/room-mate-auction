...
/* Global overrides */
.stApp, [data-testid="stAppViewContainer"] { font-family: 'Inter', sans-serif !important; }

/* Match result hero card */
.hero-match {
    background: linear-gradient(135deg, rgba(6,182,212,0.12) 0%, rgba(14,116,144,0.12) 100%);
...
.badge-fire { background: #06b6d4; }
.badge-great { background: #0891b2; }
.badge-ok { background: #0e7490; }
...
.room-card:hover { border-color: rgba(6,182,212,0.3); }
.room-card.room-me { border-color: rgba(6,182,212,0.4); background: rgba(6,182,212,0.06); }
.room-card h4 { margin: 0 0 6px 0; color: #94a3b8; font-size: 0.8rem; text-transform: uppercase; letter-spacing: 0.05em; font-weight: 600; }
.room-card .room-names { font-size: 1.15rem; font-weight: 600; color: #f1f5f9; }
.room-card .room-meta { color: #64748b; font-size: 0.8rem; margin-top: 6px; }
...
def render_sidebar():
    with st.sidebar:
        st.markdown("### :material/heart_favorite: Roommate Auction")
        st.divider()
        ...
        if st.session_state.auction_id:
            ...
            if config:
                ...
                st.caption(f"Auction code: `{st.session_state.auction_id}`")
                st.divider()
...
def show_home():
    ...
    st.markdown("## :material/heart_favorite: Roommate Auction")
    st.caption("Create your own auction or join an existing one")
    st.divider()
    ...
    st.divider()
    st.caption("Made with :material/favorite: by **Atharv**  |  Follow on Instagram: [@atharvv_t](https://instagram.com/atharvv_t)")
...
def show_login():
    ...
    st.markdown(f"## :material/heart_favorite: {config['group_name']}")
    ...
    if locked:
        with st.container(border=True):
            st.success("Auction complete. Select your name below to see your match.")
    elif all_submitted:
        with st.container(border=True):
            st.success("All profiles submitted!")
            if st.button(":material/lock: Lock & Run Match", type="primary", use_container_width=True):
                ...
                if ok:
                    st.toast("Match complete!", icon="\u2705"); time.sleep(0.5); st.rerun()
                else:
        ...
        with st.container(border=True):
            ...
            cols = st.columns(min(len(config["participants"]), 4))
            for idx, p in enumerate(config["participants"]):
                with cols[idx % len(cols)]:
                    if p in submitted_names:
                        st.markdown(f'<span class="pill pill-done">:material/check_circle: {p}</span>', unsafe_allow_html=True)
                    else:
                        st.markdown(f'<span class="pill pill-wait">:material/schedule: {p}</span>', unsafe_allow_html=True)
...
def show_preferences():
    ...
    st.markdown(f"## :material/person: {user}'s Preferences")
...
def show_submitted():
    ...
    st.markdown(f"## :material/check_circle: Thanks, {user}")
    st.divider()

    with st.container(border=True):
        st.success("Your preferences are locked in. No one can see your ratings.")
...
def show_results():
    ...
    st.markdown(f"## :material/celebration: {user}'s Match")
    ...
    <div class="hero-match">
        <div style="font-size:0.8rem;color:#06b6d4;text-transform:uppercase;letter-spacing:0.1em;font-weight:600">Your Roommate</div>
        <div class="hero-names">{user} &mdash; {my_roommate}</div>
        <div style="margin:12px 0"><span class="badge {badge_cls}">{badge} &middot; {sc:.1f}/5.0</span></div>
        <div class="hero-sub">
            You rated {my_roommate}: <b>{'&#9829;' * sa} ({sa}/5)</b> &nbsp;&middot;&nbsp;
            {my_roommate} rated you: <b>{'&#9829;' * sb} ({sb}/5)</b>
        </div>
        <div class="hero-sub">Room #{room_num}</div>
    </div>
    ...
    st.divider()
    st.caption(":material/lock: Your preferences and scores are private. Only you can see your individual ratings.")
    if st.button(":material/logout: Sign out"):
        st.session_state.user = None; st.session_state.page = "login"; st.rerun()
...
def show_admin():
    ...
    if not config:
        st.error("Auction not found.")
        if st.button(":material/arrow_back: Back"):
            st.session_state.page = "home"; st.session_state.auction_id = None; st.session_state.auction_id = None; st.rerun()
        return

    st.markdown(f"## :material/admin_panel_settings: Admin Dashboard")
    ...
    m1.metric("Submissions", f"{len(prefs)}/{len(participants)}")
    m2.metric("Status", "Locked" if is_locked(aid) else "Open")
    m3.metric("Results", "Ready" if results else "Pending")
    ...
    if len(prefs) >= 2:
        ...
        for i, room in enumerate(preview_rooms):
            ...
            st.markdown(f'<div class="room-card"><h4>Room {i+1} <span style="text-transform:none;letter-spacing:normal;font-weight:400;color:#64748b">(preview)</span></h4><div class="room-names">{members_str}</div><span class="badge {badge_cls}" style="margin-top:6px">{sc:.1f}/5.0</span></div>', unsafe_allow_html=True)
    ...
    if len(prefs) >= 2:
        ...
        with st.expander(":material/radar: Preference Profiles"):
            ...
            for idx, person in enumerate(radar_people):
                ...
                if values:
                    ...
                    fig_r = go.Figure(go.Scatterpolar(r=values, theta=others_closed, fill='toself', line=dict(color='#06b6d4'), fillcolor='rgba(6,182,212,0.15)'))
                    fig_r.update_layout(polar=dict(radialaxis=dict(visible=True, range=[0, 5], color='#94a3b8'), angularaxis=dict(color='#94a3b8'), bgcolor='rgba(0,0,0,0)'), showlegend=False, height=300, margin=dict(t=20, b=20, l=40, r=40), paper_bgcolor='rgba(0,0,0,0)')
                    with cols_r[idx % len(cols_r)]:
                        st.markdown(f"**{person}**")
                        st.plotly_chart(fig_r, use_container_width=True)

        # Pickiness & Friendliness
        ...
        with st.expander(":material/favorite: Dream Pairs vs :material/skull: Bottom Pairs"):
            ...
            with col_d:
                ...
                for _, row in df_pairs.head(5).iterrows():
                    st.markdown(f'<div style="background:rgba(6,182,212,0.08);border-left:3px solid #06b6d4;padding:0.5rem 0.8rem;border-radius:6px;margin:0.25rem 0;color:#f1f5f9"><b>{row["Pair"]}</b> &mdash; {row["Score"]:.1f}</div>', unsafe_allow_html=True)
            with col_n:
                st.markdown("**Bottom Pairs**")
                for _, row in df_pairs.tail(5).iterrows():
                    st.markdown(f'<div style="background:rgba(239,68,68,0.08);border-left:3px solid #ef4444;padding:0.5rem 0.8rem;border-radius:6px;margin:0.25rem 0;color:#fca5a5"><b>{row["Pair"]}</b> &mdash; {row["Score"]:.1f}</div>', unsafe_allow_html=True)
        ...
        with st.expander(":material/account_tree: Preference Flow Network"):
            ...
            st.caption("Lines show ratings of 3+. Thicker = stronger preference.")
    ...
    if results:
        ...
        for i, room in enumerate(results["rooms"]):
            ...
            badge_cls = 'badge-fire' if sc >= 4 else 'badge-great' if sc >= 3.5 else 'badge-ok'
            st.markdown(f'<div class="room-card"><h4>Room {i+1}</h4><div class="room-names">{names_html}</div><span class="badge {badge_cls}" style="margin-top:6px">{sc:.1f}/5.0</span><div class="room-meta">{indiv}</div></div>', unsafe_allow_html=True)
        ...
        # Monte Carlo
        with st.expander(":material/casino: Simulation (1000 rounds)", expanded=True):
            sim_scores = []
            pts = participants[:]
            for _ in range(1000):
                random.shuffle(pts)
    ...
    with c1:
        if st.button(":material/lock: Force Lock & Match", type="primary", use_container_width=True):
            ok, missing = lock_auction(aid)
            if ok: st.toast("Locked!", icon="\u2705"); st.rerun()
    ...
    with c2:
        if st.button(":material/delete: Delete Auction", use_container_width=True):
            ...
            st.toast("Deleted", icon="\u2705"); st.session_state.auction_id = None; st.session_state.page = "home"; st.rerun()
